...
class OrderCheckoutActivity : AppCompatActivity() {
    ...
    private val filtered: ArrayList<OrderListResponse>? by lazy {
            intent.extras?.getParcelableArrayList<OrderListResponse>("PRODUCT_FILTERED")
        }
    ...
    private fun initAdapterProducts() {
        with(rvOrderSummary) {
            adapter = filtered?.let { OrderCheckoutAdapter(it) }
            }
        }
